...
//    private static final String PRODUCT_FORM = "productForm-new.jsp";
    ...
    private String handleProductAdding(HttpServletRequest request, HttpServletResponse response) {
        if (AuthUtils.isAdmin(request)) {
            ...
            //check size
            if (!size.matches("^(X|XS|S|M|L|XL|XXL|XXXL)$")) {
                checkError += "<br/> Invalid Size.";
            }
            //check name 
//            if(){
//                
//            }
            //check image
            if(!image.matches(".*\\.(jpg|jpeg|png|gif)$")){
               checkError += "<br/> Image must be in jpg, jpeg, png or gif format.";
